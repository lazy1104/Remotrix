...
tab-url = 链接任务
tab-torrent = 种子任务
tab-metalink = Metalink 任务
drop-torrent-hint = 拖入 .torrent 文件，或点击选择文件
drop-torrent-active = 松开以添加种子文件
drop-metalink-hint = 拖入 .metalink / .meta4 文件，或点击选择文件
...
link-type-metalink = Metalink 链接
webpage-filter = 普通网页 URL 过滤
webpage-filter-smart = 智能（推荐）
webpage-filter-static = 仅静态识别
webpage-filter-off = 关闭
...
aria2-update-starting-title = 正在更新 aria2-next
...
update-beta-channel = 包含预发布版本更新 (Beta)
update-beta-channel-hint = 同时提供 Remotrix 应用的预发布 (Beta) 版本
...
proxy-address-placeholder = http://主机:端口
proxy-username-placeholder = 账号（可选）
proxy-password-placeholder = 密码（可选）
proxy-protocol-hint = 普通下载仅支持 HTTP；BitTorrent 支持 HTTP 或 SOCKS
...
metalink = Metalink
follow-metalink = 自动展开 Metalink
follow-metalink-hint = 启用后，aria2 会将 Metalink 响应展开为多个子下载任务。关闭后只下载 Metalink 文件本身。
ed2k-search = ED2K 搜索
ed2k-search-keyword = 关键词
ed2k-search-file-type = 文件类型
ed2k-search-min-sources = 最少来源数
ed2k-search-timeout = 超时（秒）
ed2k-search-submit = 开始搜索
ed2k-search-cancel = 取消搜索
...
ed2k-bootstrap-sync-interval = 同步间隔（小时）
ed2k-bootstrap-sync-now = 立即同步
ed2k-bootstrap-sync = ED2K 引导同步
...
ed2k-bootstrap-server-met-modified = server.met 修改时间
ed2k-bootstrap-nodes-dat-modified = nodes.dat 修改时间
ed2k-bootstrap-synced = server.met / nodes.dat 已同步
ed2k-bootstrap-sync-failed = 引导文件同步失败
ed2k-bootstrap-managed-default-hint = 当上方服务器列表 / 节点列表为空时，将自动使用内置的默认 server.met / nodes.dat，修改后需重启引擎生效
ed2k-bootstrap-cache-status = 缓存：server.met { $server-met-size } · nodes.dat { $nodes-dat-size }
task-kind-ed2k = ED2K
task-kind-metalink = Metalink
paths-section-title = 路径
path-aria2-dir-label = aria2-next 目录
path-app-data-label = 应用缓存目录
path-log-dir-label = 日志目录
path-restore-default = 恢复默认
path-restart-hint = 修改后需要重启应用才能生效，启动时会自动把数据复制到新路径
