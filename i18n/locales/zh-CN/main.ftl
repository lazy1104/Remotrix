...
confirm-close-tray-pref-hint = 之后关闭将直接最小化到托盘，不再询问。可在设置中修改；需启用托盘。
close-action = 直接关闭
tray-action = 最小化到托盘
tray = 托盘
tray-enabled = 显示系统托盘图标
close-to-tray = 关闭时最小化到托盘
close-to-tray-hint = 启用后，关闭窗口将直接最小化到托盘，不再弹出确认对话框。
tray-restart-hint = 启用托盘需重启应用后生效。
tray-show = 显示主窗口
tray-hide = 最小化到托盘
