...
webpage-filter = Webpage URL filter
webpage-filter-smart = Smart (recommended)
webpage-filter-static = Static only
webpage-filter-off = Off
...
proxy-address-placeholder = http://host:port
...
ed2k-bootstrap-managed-default-hint = Built-in default server.met / nodes.dat is used when the server / node list fields above are empty. Changes take effect after the engine restarts.
ed2k-bootstrap-cache-status = Cache: server.met { $server-met-size } · nodes.dat { $nodes-dat-size }
task-kind-ed2k = ED2K
task-kind-metalink = Metalink
paths-section-title = Paths
path-aria2-dir-label = aria2-next directory
path-app-data-label = App cache directory
path-log-dir-label = Log directory
path-restore-default = Restore default
path-restart-hint = Changes take effect on restart. Existing data is copied automatically on next launch.
