...
- **No bundled CJK font.** The binary no longer embeds a CJK fallback font (`fonts/HarmonyOS_Sans_SC_Regular.ttf` removed, `src/main.rs` no longer `include_bytes!`s it). Default `font_family` is empty → `iced::Font::DEFAULT`; on first launch `config::migrate_font_family` (`src/config/migrations.rs`) writes the result of `system_fonts::find_for_system_locale(FontStyle::Sans)` into `Settings.font_family` (cached in `src/ui/font_autopick.rs`). On a Linux container without CJK fonts installed (e.g. `scmilinux` + Chinese task names) the chosen family may not cover CJK glyphs and tasks render as `.notdef` (tofu); users can install `fonts-noto-cjk` or pick a CJK family in Settings → Appearance → Font. Verify the binary shrank with `strings target/release/remotrix | grep -c HarmonyOSSansSC` — should be 0 (was nonzero before this change).
