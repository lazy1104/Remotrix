...
- **Auto-retrying error tasks across restarts.** aria2's `--input-file`/`--save-session` (`src/engine.rs:379-384`) restore every saved entry on startup and rewrite it every 5 s. When a task transitions to `Error` *without* being paused, aria2's session write omits `pause=true`, so the next startup re-adds the same dead task → it errors again → written again. The per-session `max-tries` / `retry-wait` settings (`src/ui/settings_page.rs:995-1009`) do **not** cap cross-restart retries — each engine lifetime gets its own 5-tries budget. To stop the loop, `prune_error_tasks_from_session` (`src/engine.rs`, called from `Sidecar::spawn` right before `--input-file`) parses `session.txt`, looks up each `gid=` in our SQLite via `db::is_task_error`, and strips any block whose DB status is `error` before aria2 sees the file. The task still lives in our DB (UI shows it as Error so the user can delete or "重新下载") but aria2 no longer auto-resumes it. Any failure in the prune path logs `warn!` and is a no-op, so a corrupt `session.txt` or unreachable DB cannot block engine spawn.
